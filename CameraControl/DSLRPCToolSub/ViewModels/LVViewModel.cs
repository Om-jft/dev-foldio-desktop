...
{
    public class LVViewModel : ViewModelBase
    {
        ...
        private string _recButtonText = TranslationStrings.LabelRecordVideo;
        ...
        private Timer _recordingTimer = new Timer(1000);
        private bool _isEnableRecordingTimer = false;
        ...
        public bool IsEnableRecordingTimer
        {
            get { return _isEnableRecordingTimer; }
            set
            {
                _isEnableRecordingTimer = value;
                RaisePropertyChanged(() => IsEnableRecordingTimer);
            }
        }
        ...
        public BitmapSource Bitmap
        {
            get
            {
                if (_bitmap == null) { _bitmap = new BitmapImage(new Uri("pack://application:,,,/Images/NoImage.png")); }
                return _bitmap;
            }
            set
            {
        ...
        public bool Recording
        {
            ...
            set
            {
                ...
                IsEnableRecordingTimer = _recording;
            }
        }
        ...
        private static LVViewModel uni_class_inst = null;
        public static LVViewModel lvInstance()
        {
            if (uni_class_inst == null)
                uni_class_inst = new LVViewModel();

            return uni_class_inst;
        }

        public void LoadIntialize()
        {
        ...
        public void LoadIntialize(ICameraDevice device)
        {
            CameraDevice = device;
            //_window = window;
        ...
        private void CaptureSnapshotThread()
        {
            ...
            {
                Log.Debug("Error capture snapshot", e);
            }
        ...
        private void Init()
        {
            ...
            if (_timer != null) { _timer.Stop(); } else { _timer = new Timer(1000 / DesiredFrameRate); }
        ...
        public void UnInit()
        {
            ...
            ServiceProvider.DeviceManager.LiveViewImage[CameraDevice] = null;
            Bitmap = null;
            LiveViewData = null;
            ...
            _timer =  new Timer(1000 / DesiredFrameRate); ;
        }
        ...
        public void WindowsManager_Event(string cmd, object o)
        {
            ...
            switch (cmd)
            {
                ...
                case CmdConsts.LiveView_Zoom_All:
                    if (CameraDevice.LiveViewImageZoomRatio != null)
                    {
                        CameraDevice.LiveViewImageZoomRatio.SetValue(0);
                    }
                    break;
                case CmdConsts.LiveView_Zoom_5x:
                    if (CameraDevice.LiveViewImageZoomRatio != null)
                    {
                        CameraDevice.LiveViewImageZoomRatio.SetValue(5);
                    }
                    break;
                case CmdConsts.LiveView_Zoom_10x:
                    if (CameraDevice.LiveViewImageZoomRatio != null)
                    {
                        CameraDevice.LiveViewImageZoomRatio.SetValue(10);
                    }
                    break;
        ...
        {
            TimeSpan diff = DateTime.Now - _recordStartTime;
            RecordingTime = diff.ToString(@"hh\:mm\:ss");
        }
        ...
        public string DrawLevelGraphBitmap(Bitmap bmp)
        {
            string outfile = Path.Combine(Settings.ApplicationTempFolder, "LV00001.jpg");
            try
            {
                try
                {
                    if (File.Exists(outfile)) { File.Delete(outfile); ; Thread.Sleep(100); }
                }
                catch (Exception)
                {
                    Thread.Sleep(100);
                ...
                WriteableBitmap writeableBitmap = BitmapFactory.ConvertToPbgra32Format(BitmapSourceConvert.ToBitmapSource(bmp));
                writeableBitmap.Freeze();

                byte[] _returnValue = StaticClass.SaveJpeg(writeableBitmap);
                File.WriteAllBytes(outfile, _returnValue);
            }
            catch (Exception ex) { }
            return outfile;
        }
        ...
        public void ProcessLiveView(Bitmap bmp)
        {
            ...
            if (_totalframes % DesiredFrameRate == 0 && ShowHistogram)
            {
                ...
                var outfile = DrawLevelGraphBitmap(bmp);
                //PathUpdate.getInstance().PathCaptureImg = bmp;
                PathUpdate.getInstance().PathCaptureImg = outfile;
            }
            ...
            if (CameraDevice.LiveViewImageZoomRatio == null) { return; }
            ...
            //if (StaticClass.__Ratio_Diff_Width > 0 || StaticClass.__Ratio_Diff_Height > 0)
            //{
            //    if (StaticClass.__Ratio_Diff_Width > 0) { CropOffsetX = StaticClass.__Ratio_Diff_Width / 2; } else { CropOffsetX = 0; };
            //    if (StaticClass.__Ratio_Diff_Height > 0) { CropOffsetY = StaticClass.__Ratio_Diff_Height / 2; } else { CropOffsetY = 0; }

            //    writeableBitmap = writeableBitmap.Crop(CropOffsetX, CropOffsetY, writeableBitmap.PixelWidth, writeableBitmap.PixelHeight);
            //}
        ...
        public byte[] SaveJpeg(WriteableBitmap image)
        {
            ...
            using (MemoryStream stm = new MemoryStream())
            {
                ...
                {
                    Log.Error("Unable to save img", e);
                }
                ...
                case 2:
                    {
                        for (int i = 1; i < 6; i++)
                        {
                            writeableBitmap.DrawLine(0, (int)((writeableBitmap.Height / 6) * i),
                                (int)writeableBitmap.Width,
                                (int)((writeableBitmap.Height / 6) * i), color);
                            writeableBitmap.DrawLine((int)((writeableBitmap.Width / 6) * i), 0,
                                (int)((writeableBitmap.Width / 6) * i),
                                (int)writeableBitmap.Height, color);
                        }
                        writeableBitmap.SetPixel((int)(writeableBitmap.Width / 2), (int)(writeableBitmap.Height / 2), 128,
                            Colors.Red);
                ...
                case 3:
                    {
                        ...
                        writeableBitmap.DrawLineDDA(0, 0, (int)writeableBitmap.Width, (int)writeableBitmap.Height, color);
                        writeableBitmap.DrawLineDDA(0, (int)writeableBitmap.Height, (int)writeableBitmap.Width, 0, color);
        ...
        private void RecordMovieThread()
        {
            try
            {
                ...
                Thread.Sleep(3000);
                TurnTableViewModel.GetInstance().VideoMode_Right();
                RecordingTime = "00:00:00";
                Recording = true;
                _recordStartTime = DateTime.Now;
                _recordingTimer.Start();
                DSLR_Tool_PC.StaticClass.IsCapturedPhoto = false;
        ...
        private void StopRecordMovieThread()
        {
            try
            {
                ...
                TurnTableViewModel.GetInstance().StopDeviceProcessing();
                _recordingTimer.Stop();
                //  do { Thread.Sleep(100); } while (DSLR_Tool_PC.StaticClass.IsCapturedPhoto == false);
                RecordingTime = "";
                Recording = false;
            }
            catch (Exception exception)
        ...
        public void StartLiveView()
        {
            try
            {
                try { _timer.Stop(); } catch (Exception ex) { }
                if (!IsActive)
                    return;
        ...
        private void StartLiveViewThread()
        {
            try
            {
                ...
                {
                    if (CameraDevice.DeviceName != null && CameraDevice.SerialNumber != null)
                    {
                        _timer.Start();
                    }
                }
        ...
        public void Capture(bool preview = false)
        {
        ...
        public void ShowLiveImagePart(bool _visibility)
        {
            _isLiveImageShow = _visibility;
            IsLiveImageShow = _visibility;
        }

        public bool IsLiveImageShow
        {
            get { return _isLiveImageShow; }
            set
            {
                _isLiveImageShow = value;
                RaisePropertyChanged(() => IsLiveImageShow);
            }
        }
        private bool _isLiveImageShow = false;

        public void StopStartLievView()
        {
            _timer.Stop();
            _timer.Start();
            ////Task.Factory.StartNew(GetLiveImage);
        ...
        public System.Windows.Media.Stretch _IsStretchToFill = System.Windows.Media.Stretch.Fill;
        public System.Windows.Media.Stretch IsStretchToFill
        {
